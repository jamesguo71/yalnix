...
typedef struct {
    pcb_node_t *sentinel_node;
} pcb_node_list;

int empty(pcb_node_list *l)
{
    return (l->sentinel_node->next == l->sentinel_node);
}

void delete_node(pcb_node_t *node)
{
...
pcb_node_t *first(pcb_node_list *l) {
    return l->sentinel_node->next;
}

pcb_node_t *sentinel(pcb_node_list *l) {
    return l->sentinel_node;
}

pcb_node_t *next(pcb_node_t *n) {
    return n->next;
}


pcb_node_t *insert_before(pcb_node_t *node, pcb_t *pcb)
{
    pcb_node_t *newnode;
    pcb_node_t *prev_node = node->prev;

    newnode = (pcb_node_t *) malloc (sizeof(pcb_node_t));
    if (newnode == NULL) {
...
int append(pcb_node_list *l, pcb_t *pcb)
{
    if (insert_before(l->sentinel_node, pcb) == NULL) {
        return ERROR;
    }
    return SUCCESS;
}

pcb_node_list *pcb_node_list_new()
{
    pcb_node_list *d;
    pcb_node_t *node;

    d = (pcb_node_list *) malloc (sizeof(pcb_node_list));
    node = (pcb_node_t *) malloc(sizeof(pcb_node_t));
...
void pcb_node_list_free(pcb_node_list *l)
{
    while (!empty(l)) {
        delete_node(first(l));
    }
...
int    ProcAdd(pcb_node_list *list, pcb_t *_proc) {
    // 1. Check arguments. Return error if invalid.
    if (!list || !_proc) {
        TracePrintf(1, "ProcAdd: Invalid list or process pointer\n");
        return ERROR;
    }
    if (append(list, _proc) == ERROR) {
        TracePrintf(1, "ProcAdd failed.\n");
        return ERROR;
    }
    return SUCCESS;
}

pcb_t *ProcGet(pcb_node_list *list, int pid) {
    // 1. Check arguments. Return error if invalid.
    if (!list || pid < 0) {
        TracePrintf(1, "[ProcGet] Invalid list or pid\n");
        return NULL;
    }

    for (pcb_node_t *cur = first(list); cur != sentinel(list); cur = next(cur)) {
        if (cur->pid == pid)
            return cur->pcb;
    }

    // 3. If we made it this far, then the process specified by _pid was not found. Return NULL.
    TracePrintf(1, "[ProcGet] Process %d not found in process list\n", pid);
    return NULL;
}
/*
 * Remove the next available node from the list and return its corresponding pcb
 */
pcb_t *ProcNext(pcb_node_list *list) {
    // 1. Check arguments. Return error if invalid.
    if (!list) {
        TracePrintf(1, "[ProcNext] Invalid list pointer\n");
        return NULL;
    }

    // 2. Check to see that we actually have processes in our list. If not, return NULL
    if (empty(list)) {
        TracePrintf(1, "[ProcNext] list is empty\n");
        return NULL;
    }
    pcb_node_t *first_node = first(list);
    pcb_t *proc = first_node->pcb;
    delete_node(first_node);
    return proc;
}

int    ProcPrint(pcb_node_list *list)
{
    if (!list) {
        TracePrintf(1, "[ProcPrint] Invalid list addr\n");
        return ERROR;
    }
...
int    ProcRemove(pcb_node_list *list, int pid) {
    if (!list || pid < 0) {
        TracePrintf(1, "[ProcRemove] Invalid list or pid\n");
        return ERROR;
    }
    ...
    // 3. If we made it this far, then the process specified by pid was not found.
    TracePrintf(1, "[ProcRemove] Process %d not found in process list\n", pid);
    return ERROR;
}
...
typedef struct proc_list {
    pcb_node_list *blocked_list;
    pcb_node_list *ready_list;
    pcb_node_list *terminated_list;
    pcb_node_list *processes_list;
    pcb_t *running;
} proc_list_t;
...
proc_list_t *ProcListCreate() {
    ...
    proc_list->blocked_list = pcb_node_list_new();
    proc_list->ready_list = pcb_node_list_new();
    proc_list->terminated_list = pcb_node_list_new();
    proc_list->processes_list = pcb_node_list_new();
 ...
 */
int ProcListDelete(proc_list_t *proc_list) {
    // 1. Check arguments. Return error if invalid.
    if (!proc_list) {
    ...
    pcb_node_list_free(proc_list->blocked_list);
    pcb_node_list_free(proc_list->ready_list);
    pcb_node_list_free(proc_list->terminated_list);
    pcb_node_list_free(proc_list->processes_list);
    proc_list->running          = NULL;
...
//    return ProcAdd(_proc_list->blocked_list, _process);
//}
//
//
///*!
...
//    return ProcGet(_proc_list->blocked_list, _pid);
//}
//
//
///*!
...
int ProcListProcessAdd(proc_list_t *_proc_list, pcb_t *_process) {
    ...
    return ProcAdd(_proc_list->processes_list, _process);
}
//
//
///*!
...
int ProcListProcessPrint(proc_list_t *_proc_list) {
    ...
    return ProcPrint(_proc_list->processes_list);
}
//
//
///*!
...
pcb_t *ProcListReadyNext(proc_list_t *_proc_list) {
    ...
    return ProcNext(_proc_list->ready_list);
}
...
int ProcListReadyPrint(proc_list_t *_proc_list) {
    ...
    return ProcPrint(_proc_list->ready_list);
}
//
//
///*!
